...
echo ""
echo "빌드된 이미지:"
docker images --filter "reference=${FULL_IMAGE}*"
...
echo "[1/2] 통합 설치 관리자 태그 업데이트 및 이미지 푸시 중..."
docker tag ${FULL_IMAGE}:${VERSION} ${FULL_IMAGE}:latest
docker push ${FULL_IMAGE}:latest
if [ $? -eq 0 ]; then
    echo "✓ 통합 설치 관리자 latest 이미지 푸시 성공"
else
    echo "✗ 통합 설치 관리자 latest 이미지 푸시 실패"
    exit 1
fi
