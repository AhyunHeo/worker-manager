...
'@
    ...
    [System.Windows.Forms.MessageBox]::Show(
        "Worker Manager 설치 완료!`n`n접속 주소:`n- 메인 페이지: http://${detectedIP}:5000/central`n- 워커노드 설정: http://${detectedIP}:5000/worker/setup`n- 관리자 대시보드: http://${detectedIP}:5000/dashboard`n`n설치 경로: $installDir`n로그 파일: $logFile",
        'Success',
        'OK',
        'Information'
    )
