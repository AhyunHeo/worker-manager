...
""".format(
    ...
    # === 시스템 환경 진단 (시작도 안되는 경우 대비) ===
    batch_lines.append('REM === System environment diagnostics ===')
    batch_lines.append('echo [%time%] Checking system environment... >> "%LOGFILE%"')
    batch_lines.append('')

    # TEMP 디렉토리 확인
    batch_lines.append('if not defined TEMP (')
    batch_lines.append('    echo [%time%] ERROR: TEMP environment variable is not defined >> "%LOGFILE%"')
    batch_lines.append('    set "TEMP=%USERPROFILE%\\AppData\\Local\\Temp"')
    batch_lines.append('    echo [%time%] Using fallback TEMP: %TEMP% >> "%LOGFILE%"')
    batch_lines.append(')')
    batch_lines.append('if not exist "%TEMP%" (')
    batch_lines.append('    echo [%time%] ERROR: TEMP directory does not exist: %TEMP% >> "%LOGFILE%"')
    batch_lines.append('    mkdir "%TEMP%" 2>nul')
    batch_lines.append('    echo [%time%] Created TEMP directory >> "%LOGFILE%"')
    batch_lines.append(')')
    batch_lines.append('')

    # PowerShell 버전 및 실행 정책 확인
    batch_lines.append('echo [%time%] Checking PowerShell version... >> "%LOGFILE%"')
    batch_lines.append('powershell -NoProfile -Command "$PSVersionTable.PSVersion.ToString()" >> "%LOGFILE%" 2>&1')
    batch_lines.append('if %ERRORLEVEL% NEQ 0 (')
    batch_lines.append('    echo [%time%] ERROR: PowerShell is not available or not working properly >> "%LOGFILE%"')
    batch_lines.append('    echo ERROR: PowerShell is not working. Please ensure PowerShell is installed.')
    batch_lines.append('    pause')
    batch_lines.append('    exit /b 1')
    batch_lines.append(')')
    batch_lines.append('')

    # Windows Forms 어셈블리 로드 테스트
    batch_lines.append('echo [%time%] Testing Windows Forms assembly... >> "%LOGFILE%"')
    batch_lines.append('powershell -NoProfile -ExecutionPolicy Bypass -Command "try { Add-Type -AssemblyName System.Windows.Forms; Write-Host \'OK\' } catch { Write-Host \'FAILED: \' + $_.Exception.Message }" >> "%LOGFILE%" 2>&1')
    batch_lines.append('if %ERRORLEVEL% NEQ 0 (')
    batch_lines.append('    echo [%time%] WARNING: Windows Forms assembly test returned non-zero >> "%LOGFILE%"')
    batch_lines.append(')')
    batch_lines.append('')

    batch_lines.append('echo [%time%] System environment check completed >> "%LOGFILE%"')
    batch_lines.append('')
